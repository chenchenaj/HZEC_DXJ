<template>
	<view class='container'>
		<view class="info flex_r_s">
			...
			<view class="title-content flex_r_e">
				<view class="chose-content">
					{{(detail.type==0&&'点检')||(detail.type==1&&'巡检')||(detail.type==2&&'维修')||(detail.type==3&&'大修列表') || (detail.type==4&&'保养')||(detail.type==5&&'润滑')||(detail.type==6&&'隐患')||''}}
				</view>
			</view>
		</view>
		...
		<view class="info flex_r_s">
			...
			<view class="title-content flex_r_e">
				<view class="chose-content">
					{{(detail.priority==0&&'正常')||(detail.priority==1&&'重要')||(detail.priority==2&&'紧急重要')}}
				</view>
			</view>
		</view>
		<view v-if="detail.type!=6" class="info flex_c_s_s">
			...
			<view v-if="detail.isCycle" v-for="(item,index) in detail.time" :key='index' class="flex_r_s"
		...
		<view class="info flex_r_s">
			...
			<view class="title-content flex_r_e">
				<view class='line1' style="text-align: right; width: 400rpx; color: #626262;">{{detail.createdUserName}}
				</view>
			</view>
		</view>
		...
		<view class="info flex_c_s">
			...
			<view v-if="detail.components" v-for="(item,index) in detail.components" :key="index" style="width: 100%;">
				...
				<view v-for="(itm,idx) in item.device" :key="idx">
					...
					<view v-for="(itm1,idx1) in itm.child" :key='idx1' class="flex_c_s"
						...
						<view v-if="detail.type<4" class="flex_r_s_s" style="width: 600rpx;margin: 30rpx 0 35rpx;">
							...
							<view class="flex_c_s_s" style="width: 420rpx;">
								<view class="flex_r_s_s">
									<view class='flex_r_s'>
										<image class='chose-icon' src="../../static/check/select.png" mode="aspectFill">
										</image>
										<view v-if="detail.type==0||detail.type==1" class="chose-content font-color">
											{{(itm1.info.result==0&&'正常')||(itm1.info.result==1&&'自行解决')||(itm1.info.result==2&&'未解决,提交求助')}}
										</view>
										<view v-if="detail.type==2||detail.type==3" class="chose-content"
											style="color: #5786F7;">
											{{(itm1.info.result==0&&'完成')||(itm1.info.result==1&&'未完成')||(itm1.info.result==2&&'无法执行')}}
										</view>
									</view>
						...
						<view v-if="detail.type==2||detail.type==3" class="flex_r_s_s"
							style="width: 600rpx;margin: 30rpx 0 35rpx;">
							...
							<view class="flex_c_s_s" style="width: 420rpx;">
								<view class="flex_r_s_s">
									<view class='flex_r_s'>
										...
										<view class="chose-content" style="color: #5886F8;">{{itm1.type==0?'停修':'不停修'}}
										</view>
									</view>
								</view>
							</view>
							...
							<view class="flex_c_s_s" style="width: 420rpx;">
								<view class="flex_r_s_s">
									<view class='flex_r_s'>
										...
										<view class="chose-content" style="color: #5786F7;">
											{{(itm1.questionType==0&&'机械故障')||(itm1.questionType==1&&'电气故障')||(itm1.questionType==2&&'工艺故障')||(itm1.questionType==3&&'其他故障')||(itm1.questionType==4&&'现场判断')||(itm1.questionType==5&&'自定义')||''}}
										</view>
									</view>
								</view>
							</view>
						...
						<!-- 自定义问题类型-->
						<view v-if="itm1.questionType==5&&itm1.content" class="flex_c_s_s"
						...
						<!-- 如果是无法执行 -->
						<view v-if="itm1.info&&itm1.info.result==2&&itm1.info.reason" class="flex_c_s_s"
							style="background-color: #F2F2F2; font-size: 27rpx; width: 600rpx; color: #626262; font-size: 27rpx;margin: 30rpx 0 35rpx;">
							<view class="flex_c_c" style="width: 100%;height: 150rpx;">
								<view style="width: 100%;height: 100%;">
									{{itm1.info.reason}}
						...
						<!-- 只读 -->
						<view v-if="itm1.info&&itm1.info.assist" class='flex_r_b'
						...
						<view
							v-if="itm1.info&&((itm1.info.image&&itm1.info.image[0])||itm1.info.video||itm1.info.voice) "
							class="flex_r_s" style="width: 600rpx;margin: 0rpx 0 35rpx;">
							<view class="title" style="margin-left: 0rpx;">附件</view>
						</view>
						<!-- 子设备图片信息只读 -->
						<view v-if="itm1.info&&itm1.info.image&&itm1.info.image[0]" class="flex_r_a"
							style="width: 600rpx;">
							<view v-for="(itm2,idx2) in itm1.info.image" :key='idx2' class='child-upload-img'
								@click="previewImage1(index,idx,idx1,idx2)">
								<image class='page_img' :src="IMAGE_URL+'/'+itm2" mode="aspectFill"></image>
							</view>
						</view>
						<view v-if="itm1.info&&itm1.info.image&&itm1.info.image[0]"
							style="width: 600rpx;text-align: left; margin: 31rpx 0;color: #626262; font-size: 27rpx;">
							图片详细描述(可传4张)</view>
						<!-- 视频/音频 -->
						<view class="flex_r_b" style="width: 600rpx;">
							<!-- 视频只读 -->
							<view v-if="itm1.info&&itm1.info.video" class='child-upload-video flex_c_c'
								@click="toWebView(itm1.info.video)">
							...
							<!-- 音频只读 -->
							<view v-if="itm1.info&&itm1.info.voice" class='child-upload-video flex_c_c'
								@click="playVoice(itm1.info.voice)">
								<image style="width:100%;height:150rpx;" src="../../static/voice.png" mode="aspectFill">
								</image>
		...
		<view v-if="detail.auditInfo&&detail.auditInfo[0]" class="info flex_c_s_s">
			<view class="flex_r_b" style="width: 100%;margin: 71rpx 0 39rpx 0rpx ">
				...
				<view class="flex_r_s">
					<image @click="show(1)" src="../../static/down.png" style="width: 37rpx;height: 37rpx;"
						mode="aspectFit"></image>
					<image @click="show(0)" src="../../static/up.png"
						style="width: 37rpx;height: 37rpx;margin-left: 16rpx;margin-right: 38rpx;" mode="aspectFit">
					</image>
				</view>
			</view>
			<view v-if="showVerifyList" v-for="(item,index) in detail.auditInfo" :key='index'
				class="question-content flex_c_c"
				...
				<view class='flex_c_c' style="width: 88%;height: 110rpx;font-size: 24rpx;color: #333333;">
					<view class='line1' style="width: 100%;height: 40rpx;">审批结果:<text
							style="color: #007AFF;">{{verifyRange[item.result]}}</text>；
						审批意见: <text style="color: #007AFF;">{{item.content||'暂无'}}</text> </view>
				</view>
			</view>
		</view>
		<view v-if="detail.isAudit&&!detail.hasAudit" class="info flex_c_s_s">
			<view class="flex_r_b" style="width: 100%;margin: 71rpx 0 39rpx 0rpx ">
				<view class="title" style="margin-left: 31rpx;">审核结果</view>
				<picker class='date1' mode="selector" :range="verifyRange" @change="verifySelect"
					style="margin-right: 33rpx;">
					<view class='flex_r_b ' style="min-width:101rpx ;height: 60rpx;font-size: 27rpx;">
						<view style="min-width: 150rpx;text-align: center; color: #626262;">
							{{verifyRange[verifyIdx] ||'请选择'}}
						</view>
						<image style="width: 20rpx;height: 12rpx;margin-right: 10rpx;"
							src="../../static/check/down_arrow.png" mode="aspectFit"></image>
					</view>
		...
		<view v-if="detail.isAudit&&!detail.hasAudit" class="info flex_c_s_s">
			...
			<view class="question-content flex_c_c">
				<textarea class='question-content-input' @input="verifyInput" placeholder="请输入审核意见" value="" />
				</textarea>
			</view>
		</view>
		<view v-else-if="!detail.isCreate&&detail.content" class="info flex_c_s_s">
			<view class="title" style="margin: 71rpx 0 39rpx 31rpx ">审核意见</view>
			<view class="question-content flex_c_c">
				<view class='question-content-input'>
					{{detail.content}}
		...
		<!-- 如果是发起人可看到审核人信息 -->
		<view v-if="detail.isCreate" class="info flex_r_s">
			<view class="title ">审批人</view>
			<view class="title-content flex_r_e">
				<block v-for="(item,index) in detail.verify" :key="index">
					<view v-if="index<4" class='user-item'> {{item}}</view>
					<view v-if="index<4&&index!=detail.verify.length-1" style="color: #5786F7;">></view>
				</block>
				<view v-if="detail.verify.length-1>4" style="color: #5786F7;">...</view>
			</view>
		</view>
		<view class="flex_r_c" style="margin: 54rpx 0 250rpx;">
			<view v-if="detail.isAudit&&!detail.hasAudit" class="btn" @click="submit">提交</view>
		</view>
	</view>
</template>
...
<script>
	...
	export default {
		...
		methods: {
			...
			async submit() {
				if (this.verifyIdx == -1) {
					util.showModal('请选择审批结果')
					return
				}
				// 选择的是执行
				if (this.detail.type != 2 && this.detail.type != 3 && this.verifyIdx == 0) {
					// 审核设备临时储存
					app.globalData.submitDeviceInfo = JSON.parse(JSON.stringify(this.detail.components))
					// 审核信息
					app.globalData.verifyInfo = {
						result: this.verifyIdx,
						auditId: taskId,
						content: verifyContent || ''
					}
					util.navigate('/pages/spot_check_repair/spot_check_repair', {
						type: this.detail.type
					})
				} else {
					let res = await request.post(request.USER.VERIFY_SUBMIT, {
						result: this.verifyIdx,
						auditId: taskId,
						content: verifyContent || ''
					})
					// DEBUG && console.log('审核提交结果', res.data);
					if (res.data.statusCode == 200) {
						util.goBack()
					} else {
						util.showModal(res.data.errmsg)
