	...
	<view class='container'>
		<view class="info flex_r_s">
			...
			<view class="title-content flex_r_s">
				...
				<view class="chose-content font-color">{{isMaintain?'设备保养':'设备润滑'}}</view>
			</view>
		</view>
		<view class="info flex_r_s">
			<view class="title ">流程说明</view>
			<view class="title-content">
				<textarea auto-height class='title-content-input' @input='processInput' placeholder="请输入流程说明" :value="processContent" />
				</textarea>
			</view>
		</view>
		<!-- 优先级 -->
			...
			<view v-if="cycleIdx==0" class="flex_r_s" style="margin:41rpx 0rpx;">
				...
				<picker class='cycle' mode="selector" :range="cycleRange" @change="cycleUnitSelect">
					<view class='flex_r_b' style="min-width:101rpx ;height: 60rpx;font-size: 27rpx;">
						<view style="min-width: 50rpx;text-align: center; color: #626262;margin: 0 10rpx;">{{cycleRange[cycleUnitIdx]||updateUnit||'请选择'}}</view>
				...
				<view class="flex_r_s">
					<picker class='hour' mode="selector" :range="hourRange" @change="hourSelect">
						<view class='flex_r_b' style="min-width:140rpx;height: 60rpx;">
							<view style="color: #626262; width:110rpx; height: 60rpx;line-height: 60rpx;text-align: center;">{{aging||hourRange[hourIdx]}}</view>
		...
		<view class="info flex_c_s">
			<view class="flex_r_b" style="width: 100%;margin: 80rpx 0 35rpx;">
				<view class="title ">{{isMaintain?'设备保养':'设备润滑'}}</view>
			...
			<view v-for="(item,index) in deviceInfo" :key="index" style="width: 100%;">
				...
				<view v-for="(itm,idx) in item.device" :key="idx">
					...
					<view v-for="(itm1,idx1) in itm.child" :key='idx1' class="flex_c_s" style="width: 100%;border-bottom: 1rpx solid #E5E5E5;">
						<view style="width: 580rpx; margin:41rpx 0;font-size: 27rpx;color: #626262;" @click="toDetail(itm1.id)">
							({{idx1+1}}) {{isMaintain?'保养项目':'润滑项目'}}: {{itm1.name}}
						</view>
						<!-- <view v-if="!isMaintain" class="flex_r_s_s" style="width: 600rpx;margin: 30rpx 0 35rpx;">
							<view class="title" style="margin-left: 0rpx;">润滑类型</view>
							<view class="flex_c_s_s" style="margin-left: 10rpx;">
								<view class="flex_r_s_s">
									<view class='flex_r_s' @click="checkEvent1(index,idx,idx1,0)">
										<image class='chose-icon' :src="itm1.childType==0?'../../static/check/select.png':'../../static/check/unselect.png'"
										 mode="aspectFill"></image>
										<view :class="[itm1.childType==0?'font-color':'un-font-color']" class="chose-content" style="margin-left: 10rpx;font-size: 22rpx;">补加</view>
									</view>
									<view class='flex_r_s' style="margin-left: 15rpx;" @click="checkEvent1(index,idx,idx1,1)">
										<image class='chose-icon' :src="itm1.childType==1?'../../static/check/select.png':'../../static/check/unselect.png'"
										 mode="aspectFill"></image>
										<view :class="[itm1.childType==1?'font-color':'un-font-color']" class="chose-content" style="margin-left: 10rpx;font-size: 22rpx;">更换</view>
									</view>
								</view>
							</view>
						</view> -->
		...
		<view class="info flex_c_s_s">
			...
			<view class="question-content flex_c_c">
				<textarea class='question-content-input' @input="questionInput" placeholder="请输入问题描述" :value="questionDesc" />
				</textarea>
			</view>
		</view>
		<!-- 图片/视频/音频 选择 -->
		<view class="info flex_c_s_s">
			...
			<view class="upload flex_r_b">
				<view class='upload-video flex_c_s' @click="chooseVideo">
					...
					<video class='video' v-if="video" :src="video" controls loop></video>
	...
	export default {
		data() {
			return {
				...
				typeIdx: 0, //类型选择
				priorityIdx: 0, //优先级选择
				hourRange: [1, 24, 48, 72], //执行时效范围
				hourIdx: 0, //执行时效选择
				cycleRange: [], //周期单位范围
				cycleIdx: 0, //检查周期选择0周期1临时
		...
		onLoad(params) {
			...
			recorderManager.onStop(function(res) {
				console.log('音频临时路径' + res.tempFilePath);
				if (timer) {
					clearInterval(timer)
				}
				self.isRecording = 0
		...
		methods: {
			...
			toDetail(id) {
				...
				app.globalData.isPoint = isMaintain ? 4 : 5
			...
			checkEvent1(idx1, idx2, idx3, idx) {
				if (this.deviceInfo[idx1].device[idx2].child[idx3].childType != idx) {
					this.deviceInfo[idx1].device[idx2].child[idx3].childType = idx
				}
			...
			chooseImg(idx) {
				...
				uni.chooseImage({
					...
					success: function(res) {
						...
						// 改为本地
						self.isLocalImage = 1
						DEBUG && console.log('图片列表', self.imgList);
					}
				})
			},
			// 删除图片
