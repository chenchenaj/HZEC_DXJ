	...
	export default {
		...
		methods: {
			...
			toDetail(id, uid, idx) {
				...
				} else if (this.list[idx].type == 4 || this.contentList[idx].type == 5) {
			...
			async getMyTask() {
				util.showLoading()
				let res = await request.get(request.USER.MINE_TASK, {
					type: this.typeIdx,
					page,
					limit
				})
				DEBUG && console.log('我的任务列表', res.data);
