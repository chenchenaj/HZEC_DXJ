<template>
	<view class='container'>
		...
		<!-- 原来的点巡检项目 -->
		<view v-if="oldType!=6" class="info flex_c_s">
			<view class="flex_r_s" style="width: 100%;margin: 80rpx 0 35rpx;">
				<view class="title" style="width: 400rpx; color: red;" @click="isShowDevice">{{oldTyp==0?'点检':'巡检'}}设备 <text style="color: #007AFF;">({{showOldDevice?'点击收起':'点击展开'}})</text></view>
			</view>
			<!-- 点检或巡检项目 -->
			<view v-if="oldComponents&&showOldDevice" v-for="(item,index) in oldComponents" :key="index" style="width: 100%;">
				...
				<view v-for="(itm,idx) in item.device" :key="idx">
					...
					<view v-for="(itm1,idx1) in itm.child" :key='idx1' class="flex_c_s" style="width: 100%;border-bottom: 1rpx solid #E5E5E5;">
						<view style="width: 600rpx; margin:41rpx 0;font-size: 27rpx;color: #626262;">
							({{idx1+1}}) {{oldType==0?'点检':'巡检'}}项目 : {{itm1.name}}
						</view>
						<!-- 选择检查情况 -->
						<view class="flex_r_s_s" style="width: 600rpx;margin: 30rpx 0 35rpx;">
							<view class="title" style="margin-left: 0rpx;">{{oldType==0?'点检':'巡检'}}结果</view>
							<!-- 结果 -->
							<view class="flex_c_s_s" style="width: 420rpx;">
								<view class="flex_r_s_s">
									<view class='flex_r_s'>
										...
										<view class="chose-content font-color">未解决,提交求助</view>
									</view>
								</view>
							</view>
						</view>
		...
		<view class="info flex_c_s">
			...
			<view v-for="(item,index) in deviceInfo" :key="index" style="width: 100%;">
				...
				<view v-for="(itm,idx) in item.device" :key="idx">
					<view class="flex_r_b" style="width: 100%;border-bottom: 1rpx solid #E5E5E5;">
						<view style="margin:41rpx 0rpx 41rpx 50rpx;font-size: 27rpx;color: #626262;">设备名称: {{itm.parentName}}</view>
						<image @click='deleteDevice(item.system,itm.parent)' src="../../static/check/delete.png" mode="aspectFit" style=" margin:0 38rpx 0 0;width: 40rpx;height:40rpx;"></image>
					</view>
					<view v-for="(itm1,idx1) in itm.child" :key='idx1' class="flex_c_s" style="width: 100%;border-bottom: 1rpx solid #E5E5E5;">
...
<script>
	...
	export default {
		data() {
			return {
				IMAGE_URL,
				oldType: '', //原来的（执行人无法解决的）点巡检项目类别
				showOldDevice: 0, //是否显示 （执行人无法解决的）点巡检项目
				oldComponents: [], //执行人无法解决的点巡检项目
				imgList: ['', '', '', ''],
				checkPoint: 0, //点检权限
				inspection: 0, //巡检权限
				video: '',
		...
		onUnload() {
			...
			app.globalData.isSpotCheckRepair = 0
		},
		onShow() {
			// 用户选择的系统-主设备-子设备信息
			if (app.globalData.submitDeviceInfo[0]) {
		...
		methods: {
			...
			previewImage1(idx1, idx2, idx3, idx4) {
				let list = this.oldComponents[idx1].device[idx2].child[idx3].info.image.map(item => {
			...
			// 点巡检项目是否展开
			isShowDevice() {
				this.showOldDevice = !this.showOldDevice
			},
			// 子设备检查情况
			...
			// 删除系统-主设备-子设备
			deleteDevice(system, parent) {
				delete app.globalData.deviceInfo[system].device[parent]
			...
			addPro() {
				...
				app.globalData.isSpotCheckRepair = 1
				util.navigate('/pages/device_choose/device_choose', {
					sysId: app.globalData.submitDeviceInfo[0].system
				})
			},
			// 问题描述
			questionInput(e) {
			...
			submit() {
				...
				let queryData = {
					...
					verifyInfo: JSON.stringify(app.globalData.verifyInfo)
				}
